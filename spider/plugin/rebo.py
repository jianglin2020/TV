...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        ...
        n,p=[],[]
        for o,i in enumerate(v['vod_play_list'][:1]):
            n.append(f"线路{o+1}({i.get('flag')})")
            c=[]
            print(i)
            for j in i.get('urls'):
                if bool(re.match(r'^[0-9]+$', j['name'])):
                    play_name = f"第{j['name']}集"
                else:
                    play_name = j['name']

                d={'url':j.get('url'),'p':i.get('parse_urls'),'r':i.get('referer'),'u':i.get('ua')}
                c.append(f"{play_name}${self.e64(json.dumps(d))}")
            p.append('#'.join(c))
...
if __name__ == '__main__':
    ...
    # formatJo = spider.categoryContent('10', 1, False, {})
    formatJo = spider.detailContent(['75445'])
    # formatJo = spider.playerContent('', '', False)
    # formatJo = spider.searchContent('大侦探', False)
