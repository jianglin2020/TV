...
def save_img(item, title):
    if not os.path.exists('./fty/images'):
        os.makedirs('./fty/images')
    try:
        print(f"{item['images']}")
        ...
        with open(f"./fty/images/{title}.jpg", 'wb') as f:
            f.write(res.content)
    except Exception as e:
        print(e, "图片下载失败")
...
def quark_link(name):
    ...
    for site in TARGET_SITES:
        try:
            ...
            for index, link in enumerate(extract_a_links(soup, site['url']), start=1): # 每个站点只处理前3个结果  
                save_img(link, f"{SEARCH_KEYWORD}_{site['name']}{str(index)}")
        except Exception as e:
