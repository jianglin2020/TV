...
const http = function (url, options = {}, noAuth = false, timeout = request_timeout) {
  if (options.method === "POST" && options.data) {
    ...
    if (!noAuth) {
      options.headers = Object.assign({
        'Authorization': 'alist-09ceb38a-f143-47f7-b255-c3eec819cd7bbQ8sIPnFUAETELgDMVkMGGItNb8OAsRGppwOuWVqpCdCVTfko_tThpKcbuvXJZPs'
      }, options.headers);
    }
  }
    ...
    __drives[item.name] = {
      ...
      getFile(path) {
        ...
        if(__proxy !== "" && path.includes("夸克")){
            proxy=__proxy+"&size=500000&thread=5&url="
        }
